...
int msgbox(string title /*标题*/ = "标题", string message /*信息*/ = "信息", string button /*按钮*/ = OK, std::string color = "\e[44m\e[97m", int X /*对话框左上角起始位置X坐标*/ = -1, int Y /*对话框左上角起始位置Y坐标*/ = -1, int W /*对话框长度*/ = (SIZE.ws_col / 2), int H = /*对话框宽度*/ (SIZE.ws_row / 2)) // 对话框函数
{
	...
	// 对话框边角绘制
	std::cout << "\e[" << Y << ';' << X << "H\e[30m\e[47m┌"							  // 左上角
			  << "\e[" << Y << ';' << W + X + 1 << "H\e[30m\e[47m┐"					  // 右上角
			  << "\e[" << H + Y + 1 << ';' << X << "H\e[30m\e[47m└"					  // 左下角
	...
	for (int i = X; i < W + X; i++)
	{
		std::cout << "\e[" << Y << ';' << i + 1 << "H\e[30m\e[47m─"					  // 上
				  << "\e[" << Y + H + 1 << ';' << i + 1 << "H\e[30m\e[47m─"			  // 下
				  << "\e[" << Y + H + 2 << ';' << i + 2 << "H\e[97m\e[40m " << flush; // 下阴影
	}

	std::cout << "\e[" << Y << ';' << (W - title.length()) / 2 + X << "H\e[47m\e[30m┤" << flush << " \e[31m" << title << flush << "\e[30m ├" << flush; // 标题

	// 对话框边框和阴影绘制(上下)
	for (int i = Y; i < H + Y; i++)
	{
		std::cout << "\e[" << i + 1 << ';' << X << "H\e[30m\e[47m│"					 // 左
	...
	std::cout << "\e[" << Y + H - 1 << ';' << (W - button.length()) / 2 + X << "H\e[41m\e[37m<" << button << ">\b" << flush; // 绘制按钮
